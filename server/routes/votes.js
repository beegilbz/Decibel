...
router.post('/', requireAuth, (req, res) => {
  ...
  const round = db.getRoundById(submission.roundId);
  ...
  const vote = db.castVote({ userId: req.user.id, submissionId, value });
  const allVotes = db.getVotesBySubmission(submissionId);
